...
class Spider(Spider):
  ...
  def detailContent(self, array):
    ...
    try:
      ...
      vod_play_from = '高清HD[主]$$$高清HD[备]$$$标清线路二$$$标清线路三'
      vod_play_url = (
          f'主线路${uid}$$$备用主线路${uid}_media$$$备用线路二$lemon_{uid}$$$备用线路三$sacf_{uid}'
      )
  ...
  def _extract_psch_keys(self, master_text):
    """从 master 播放列表里取出当前有效的 PSCH 密钥池。
    stripchat 会在 master m3u8 顶部列出一批 `#EXT-X-MOUFLON:PSCH:v2:<key>`，
    只有池内的 key 作为 pkey 才能拿到真正的直播分片。
    """
    return self.PSCH_PATTERN.findall(master_text or '')

  def _pick_pkey(self, master_text):
    """挑一个有效 pkey：池里有内置 key 就复用，否则换成池里第一个。
    👈 关键修复：原来 pkey 是硬编码常量，一旦被服务端轮换出池，
    CDN 不会报错，而是返回一段不含 MOUFLON:URI 的「广告占位」播放列表，
    表现就是「有时候能播、有时候播不了/黑屏」。
    """
  ...
  def _with_auth(self, variant_url, pkey):
    """给 variant 播放列表补齐鉴权参数（去掉已有的旧 pkey/psch 再拼）"""
  ...
  def playerContent(self, flag, id, vipFlags):
    ...
    try:
      ...
      # --- 新线路: doppiocdn.media (主线路2) ---
      elif id.endswith('_media'):
        real_sid = id[:-6]  # 去掉_media后缀
        rsp, pkey = self._get_master(
            f'https://edge-hls.doppiocdn.media/hls/{real_sid}/master/{real_sid}_auto.m3u8?playlistType=standard'
        )
        lines = rsp.strip().split('\n')
        for i, line in enumerate(lines):
          if '#EXT-X-STREAM-INF' in line:
      ...
      # --- 线路1: StripChat (主线路, doppiocdn.org) ---
      else:
        rsp, pkey = self._get_master(
  ...
  def update_vod(self, username):
    """👈 修复：刷新详情时剔除 vod_play_from / vod_play_url。
    壳对 refresh&type=vod 是增量合并，带上线路字段会把「主线路」一遍遍追加到选集里。
    """
    try:
  ...
  def localProxy(self, param):
    ...
    # 👈 关键修复：pkey 失效时 CDN 不报错，而是返回一段不含 MOUFLON:URI 的
    # 「广告占位」播放列表（#EXT-X-MOUFLON-ADVERT / PLAYLIST-TYPE:VOD），
    # 播放器就会卡住或黑屏。检测到这种情况就回 master 换新 pkey 重拉。
    ...
    # 👈 注意：MIME 必须是完整合法的 application/vnd.apple.mpegurl，
    # ExoPlayer 严格按此类型判断"这是需要周期刷新的直播播放列表"；
    # 拼错会导致它不刷新列表、播完初始窗口(约6秒)就停。
    return [200, 'application/vnd.apple.mpegurl', data]

  def _is_advert_playlist(self, text):
    """pkey 无效/失效时 CDN 返回的占位列表特征：有 ADVERT 标记且没有加密分片行"""
  ...
  def _retry_with_fresh_pkey(self, variant_url):
    """从 variant URL 反推 master，重新取 PSCH 池换 pkey 后重拉一次"""
    try:
      ...
      hosts = [
          ...
          'https://edge-hls.doppiocdn.media',
      ]
      for h in hosts:
        try:
  ...
  def refresh_desc_if_needed(self, room_id, force=False):
    """有新弹幕后节流刷新详情页简介。
    👈 修复两个回归：
      1. 原来用 self.stripchat_play 取房间号 —— 这个字段会被 localProxy/其他房间
         的 detailContent 覆盖，导致把「别的主播」的详情推给当前页面（选集里出现
         别人的直播）。现在直接用轮询线程自己的 room_id。
      2. 原来整包 payload 带 vod_play_from / vod_play_url，壳的 refresh 是「合并」
         而非「替换」，每刷新一次就往选集里追加一组线路 → 出现多个「主线路」。
         现在刷新只携带简介相关字段，播放线路交给首次 detailContent，不再重复下发。
    """
    now = int(time.time())
